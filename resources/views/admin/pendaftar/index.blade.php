...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="text-sm text-gray-400 bg-gray-100 px-4 py-2 rounded-full inline-flex items-center gap-2 self-start shadow-inner">
            <i class="fas fa-shield-alt text-blue-500"></i> <span>Total perlu verifikasi: {{ $pendaftar->total() ?? 0 }}</span>
        </div>
    </div>

    <!-- Filter Card (3D effect) -->
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 transition-all duration-300 hover:shadow-2xl transform hover:-translate-y-1">
        <form method="GET" action="{{ route('admin.verifikasi.index') }}" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
            <div class="md:col-span-5">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama / No Pendaftaran" class="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-400 focus:border-transparent transition shadow-sm">
            </div>
    ...
    <!-- Tabel Pendaftar (dengan 3D hover card) -->
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="group transition-all duration-200 hover:bg-gradient-to-r hover:from-blue-50 hover:to-white hover:shadow-inner transform hover:scale-[1.01] hover:rounded-xl">
                        ...
                        <td class="px-6 py-4 text-center space-y-2">
                            <a href="{{ route('admin.pendaftar.show', $p) }}" class="inline-flex items-center gap-1 bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg text-sm font-medium transition shadow-sm hover:shadow transform hover:-translate-y-0.5">
                                <i class="fas fa-eye"></i> Lihat
                            </a>
                            <form action="{{ route('admin.pendaftar.updateStatus', $p) }}" method="POST" class="inline-block">
                                @csrf
                                @method('PATCH')
                                <select name="status" class="border rounded-lg px-2 py-1 text-xs" onchange="this.form.submit()">
                                    <option value="">- Ubah Status -</option>
                                    <option value="pending" {{ $p->status == 'pending' ? 'selected' : '' }}>Pending</option>
                                    <option value="approved" {{ $p->status == 'approved' ? 'selected' : '' }}>Diterima</option>
                                    <option value="rejected" {{ $p->status == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                                </select>
                            </form>
                        </td>
                    </tr>
